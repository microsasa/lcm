 ...
 *
 */
typedef struct _udp_params_t udp_params_t;
struct _udp_params_t {
...
typedef struct _lcm_provider_t lcm_udp_t;
...
static int _setup_recv_parts(lcm_udp_t *lcm);

static GStaticPrivate CREATE_READ_THREAD_PKEY = G_STATIC_PRIVATE_INIT;

static void _destroy_recv_parts(lcm_udp_t *lcm)
{
    if (lcm->thread_created) {
        // send the read thread an exit command
        auto wstatus = lcm_internal_pipe_write(lcm->thread_msg_pipe[1], "\0", 1);
        if (wstatus < 0) {
...
void lcm_udp_destroy(lcm_udp_t *lcm)
{
...
static int parse_mc_addr_and_port(const char *str, udp_params_t *params)
{
    if (!str || !strlen(str)) {
...
{
    udp_params_t *params = (udp_params_t *) user;
...
static int _recv_message_fragment(lcm_udp_t *lcm, lcm_buf_t *lcmb, uint32_t sz)
{
...
static int _recv_short_message(lcm_udp_t *lcm, lcm_buf_t *lcmb, int sz)
{
...
static lcm_buf_t *udp_read_packet(lcm_udp_t *lcm)
{
    lcm_buf_t *lcmb = NULL;
...
static int lcm_udp_get_fileno(lcm_udp_t *lcm)
{
...
static int lcm_udp_subscribe(lcm_udp_t *lcm, const char *channel)
{
    return _setup_recv_parts(lcm);
}

static int lcm_udp_publish(lcm_udp_t *lcm, const char *channel, const void *data,
...
static int lcm_udp_handle(lcm_udp_t *lcm)
{
    int status;
    char ch;
...
static int udp_self_test(lcm_udp_t *lcm)
{
    int success = 0;
    int status;
    ...
    lcm_udp_publish(lcm, SELF_TEST_CHANNEL, (uint8_t *) msg, strlen(msg));
    ...
    do {
        ...
        if (lcm_timeval_compare(&now, &next_retransmit) > 0) {
            status = lcm_udp_publish(lcm, SELF_TEST_CHANNEL, (uint8_t *) msg, strlen(msg));
            lcm_timeval_add(&now, &retransmit_interval, &next_retransmit);
        }
...
/* Enable per-packet timestamping by the kernel, if available */
	...
	*/
    ...
    int self_test_results = udp_self_test(lcm);
    g_static_rec_mutex_lock(&lcm->mutex);
...
lcm_provider_t *lcm_udp_create(lcm_t *parent, const char *network, const GHashTable *args)
{
    udp_params_t params;
    memset(&params, 0, sizeof(udp_params_t));
    ...
    lcm_udp_t *lcm = (lcm_udp_t *) calloc(1, sizeof(lcm_udp_t));
    ...
    dbg(DBG_LCM, "Initializing LCM udp context...\n");
    dbg(DBG_LCM, "Multicast %s:%d\n", inet_ntoa(params.mc_addr), ntohs(params.mc_port));

    // setup destination UNICAST address
...
#else
static lcm_provider_vtable_t udp_vtable = {
    .create = lcm_udp_create,
    .destroy = lcm_udp_destroy,
    .subscribe = lcm_udp_subscribe,
    .unsubscribe = NULL,
    .publish = lcm_udp_publish,
    .handle = lcm_udp_handle,
    .get_fileno = lcm_udp_get_fileno,
};
#endif
...
void lcm_udp_provider_init(GPtrArray *providers)
{
    ...
    udp_vtable.create = lcm_udp_create;
    udp_vtable.destroy = lcm_udp_destroy;
    udp_vtable.subscribe = lcm_udp_subscribe;
    udp_vtable.unsubscribe = NULL;
    udp_vtable.publish = lcm_udp_publish;
    udp_vtable.handle = lcm_udp_handle;
    udp_vtable.get_fileno = lcm_udp_get_fileno;
#endif
    udp_info.name = "udp";
